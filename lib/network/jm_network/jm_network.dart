...
class JmNetwork {
  ...
  static List<String> get domains {
    var domains = appdata.appSettings.jmApiDomains
        .map((e) => e.trim())
        .where((e) => e.isNotEmpty)
        .toList();
    return domains.isEmpty ? builtInApiDomains : domains;
  }

  String get baseUrl {
    var apiDomains = domains;
    var index = int.tryParse(appdata.settings[17]) ?? 0;
    if (index < 0 || index >= apiDomains.length) {
      index = 0;
      appdata.settings[17] = "0";
    }
    return "https://${apiDomains[index]}";
  }
  ...
  Future<Res<List<JmComicBrief>>> searchNew(
    ...
    try {
      ...
      return Res(comics,
          subData: comics.isEmpty
              ? 0
              : ((res.data["total"] as num) / res.data["content"].length)
                  .ceil());
    } catch (e, s) {
      LogManager.addLog(LogLevel.error, "Data Analysis", "$e\n$s");
      Future.delayed(const Duration(microseconds: 500),
