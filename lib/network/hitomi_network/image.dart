...
class GG {
  ...
  Future<void> getGg(String galleryId) async {
    if (cacheTime != null &&
        DateTime.now().millisecondsSinceEpoch -
                cacheTime!.millisecondsSinceEpoch <
            100) {
      numbers = cacheNumbers!;
      b = cacheB!;
      return;
    }
    var dio = logDio(BaseOptions(responseType: ResponseType.plain, headers: {
      "User-Agent": webUA,
      "Referer": "https://hitomi.la/reader/$galleryId.html"
    }));
    var res =
        await dio.get<String>("https://ltn.$baseDomain/gg.js?_=1683939645979");
  ...
  String urlFromHash(HitomiFile image, String? dir, String? ext) {
    ...
    if (dir.contains('small')) {
      url = urlFromUrl(
          'https://$baseDomain/$dir/${realFullPathFromHash(image.hash)}.$ext',
          'tn');
    } else {
      url = urlFromUrl(
          'https://$baseDomain/${fullPathFromHash(image.hash)}.$ext', 'w');
    }
    return url;
  }

  ///获取图像信息
  Future<String> urlFromUrlFromHash(
      String galleryId, HitomiFile image, String? dir, String? ext) async {
